...
function copyImgs() {
    return gulp.src(paths.images, {cwd: paths.app})
        .pipe(plumber())
        .pipe(debug())
        .pipe(minifyImgs())
...
function copyFonts() {
    return gulp.src(paths.fonts, {cwd: paths.app})
        .pipe(plumber())
...
function injectFiles() {
    var sources = gulp.src(['lib/**', 'vendor/**', 'assets/**/*.css'], {cwd: paths.prod});
    return gulp.src('./index.html', {cwd: paths.prod})
        .pipe(plumber())
        .pipe(inject(sources, {addRootSlash: false}), {cwd: './www'})
...
function wiredep() {
    ...
    return gulp.src(mainBowerFiles())
        ...
        .pipe(sassCompile({outputStyle: 'compressed'}).on('error', sassCompile.logError))
        .pipe(minifyCss())
        .pipe(sourcemaps.write())
        .pipe(filterCSS.restore)
        .pipe(gulp.dest('./lib', {cwd: paths.prod}))
...
function minifyStyles() {
    return gulp.src(paths.styles, {cwd: paths.app})
        .pipe(plumber())
        .pipe(sourcemaps.init())
        .pipe(sassCompile({outputStyle: 'compressed'}).on('error', sassCompile.logError))
        .pipe(minifyCss())
